...
<div class="row g-3 mb-3">
    <div class="col-12">
        <div class="card shadow-none">
            ...
            <div class="card-body">
                <form action="{{ route('admin.state-fees.update', $stateFee) }}" method="POST">
                    @csrf
                    @method('PUT')

                    <div class="row mb-3">
                        <div class="col-md-6">
                            <x-forms.text-input name="name" label="Fee Name" :value="old('name', $stateFee->name)" required
                                help="Enter a descriptive name for this fee (e.g., California Business License, New York LLC Filing)"/>
                        </div>

                        <div class="col-md-3">
                            <x-forms.text-input name="amount" label="Initial Amount ($)" :value="old('amount', $stateFee->amount)" required
                                type="number" step="0.01" min="0.00" help="Enter the initial fee amount in dollars"/>
                        </div>

                        <div class="col-md-3">
                            <x-forms.text-input name="renew_amount" label="Renewal Amount ($)" :value="old('renew_amount', $stateFee->renew_amount)"
                                required type="number" step="0.01" min="0.00" help="Enter the renewal fee amount in dollars"/>
                        </div>
                    </div>

                    <div class="row mb-3">
                        <div class="col-md-12">
                            <x-forms.textarea name="description" label="Description" :value="old('description', $stateFee->description)"
                    ...
                    <div class="row mb-3" id="features-container">
                        ...
                        <div class="col-lg-6 feature-inputs">
                            @php
                                $oldFeatures = old('features', $stateFee->features ?? []);
                            @endphp

                            @if(is_array($oldFeatures) && count($oldFeatures) > 0)
                        ...
                        <div class="col-md-6">
                            <div class="form-check form-switch mb-3">
                                <input class="form-check-input" type="checkbox" id="status" name="status" value="1"
                                    {{ old('status', $stateFee->status ? '1' : '0') == '1' ? 'checked' : '' }}>
                        ...
                        <div class="col-md-6">
                            <div class="form-check form-switch mb-3">
                                <input class="form-check-input" type="checkbox" id="auto_renew" name="auto_renew" value="1"
                                    {{ old('auto_renew', $stateFee->auto_renew ? '1' : '0') == '1' ? 'checked' : '' }}>
                    ...
                    <div class="d-flex justify-content-end mt-4">
                        ...
                        <button type="submit" class="btn btn-primary">Update State Fee</button>
                    </div>
                </form>
            </div>
        </div>
